...
print(f'found {len(UA_flight_nums)} UA flights for Newark')


# flight_stats = "https://www.flightstats.com/v2/flight-tracker/UA/1377?year=2023&month=3&date=16&flightId=1139346788"


def scrape_flight_data(flt_num):
    today = now.strftime('%Y%m%d')
    flight_view = f"https://www.flightview.com/flight-tracker/UA/{flt_num[2:]}?date={today}&depapt=EWR"
...
with ThreadPoolExecutor(max_workers=10) as executor:
    futures = {executor.submit(scrape_flight_data, flt_num): flt_num for flt_num in UA_flight_nums}
...
# Scraping through remaining troubled
with ThreadPoolExecutor(max_workers=10) as executor:
    futures = {executor.submit(scrape_flight_data, flt_num): flt_num for flt_num in troubled}
    for future in as_completed(futures):
        flt_num = futures[future]
        try:
            result = future.result()
            ...
            troubled.append(flt_num)
# print(f'Troubled: {len(troubled)}, Master : {len(master)}')
print(f'UA flights: {len(UA_flight_nums)}; master: {len(master)}')


def ewr_UA_gate(gate):
    curated = {}
