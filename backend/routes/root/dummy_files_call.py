...
def getting_the_path_right():
    ...
    uj_trailing_path = r"Cirrostrats\backend"
    ismail_trailing_path = "Cirrostrats/backend/"
    
    trail_index = currentWorking.index('Cirrostrats')

    print(currentWorking[trail_index:])
    
    if currentWorking[trail_index:] == luis_trailing_path:
        dummy_path_to_be_used = currentWorking + "/backend/dummy_pkl/"
        print('Maybe Luis path:', dummy_path_to_be_used)
    elif currentWorking[trail_index:] == uj_trailing_path:
        dummy_path_to_be_used = currentWorking + "\\dummy_pkl\\"       # Caution! Escape char issue with `\` Windows path
        print('Maybe UJ path:', dummy_path_to_be_used)
    elif currentWorking[trail_index:] == ismail_trailing_path or currentWorking:  # This could be just `else` but elaborate for situational awareness.
        dummy_path_to_be_used = currentWorking + "/dummy_pkl/"        # linux path
...
def dummy_imports():
    ...
    def pickle_imports_and_processing():
        ...
        bulk_flight_deets_path = dummy_path_to_be_used + r"dummy_base_flight_deet_packet.pkl"
        ...
        ind = dummy_path_to_be_used + r"dummy_raw_weather_dataKIND.pkl"
        ord = dummy_path_to_be_used + r"dummy_raw_weather_dataKORD.pkl"
